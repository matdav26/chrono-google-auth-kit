...
# ⚠️ TESTING ONLY: Service key authentication helper
...
async def get_project_context(
    ...
    try:
        ...
        print(f"❌ Error in get_project_context: {e}")
...
def get_documents(
    ...
    auth_header = request.headers.get("Authorization")
    print("📄 Docs auth header:", auth_header)
...
async def upload_document_to_project(
    ...
    try:
        ...
        print(f"📁 Storage path: {storage_path}")  # Add this line to see the path
        print(f"📁 Full storage path: {full_storage_path}")  # Add this line to see the full path
        print(f"📁 Download path: {download_path}")  # Add this line to see the download path
        ...
        if "storage_path" in locals():
            try:
                ...
                print("⚠️ Failed to cleanup storage:", cleanup_err)
...
async def generate_document_summary(
    ...
    try:
        ...
        print(f"🔍 Looking for file at: {storage_path}")  # Add this line
        print(f"📄 Document data: {document}")  # Add this line to see full document
        if not storage_path:
        ...
        try:
            ...
            print(f"🔍 Download path: {download_path}")  # Add this line
            file_response = supabase.storage.from_("documents").download(download_path)
            print(f"📁 File response type: {type(file_response)}")  # Add this line
        ...
        print("❌ Exception during summary generation:", str(e))
        raise HTTPException(status_code=500, detail=str(e))
...
async def generate_event_summary(
    ...
    try:
        ...
        print(f"📄 Event response: {event_response.data}")
        ...
        print("❌ Exception during event summary generation:", str(e))
...
async def debug_event_exists(event_id: str):
    ...
    try:
        print(f"🔍 Debug: Looking for event with ID: {event_id}")
        event_response = supabase.table("events").select("*").eq("id", event_id).execute()
        print(f"📄 Debug: Event response: {event_response.data}")
...
async def debug_list_all_events():
    ...
    try:
        print("🔍 Debug: Fetching all events from database")
        events_response = supabase.table("events").select("*").execute()
        print(f"📄 Debug: Found {len(events_response.data)} events")
...
async def create_event(
    ...
    ⚠️ TESTING ONLY: This endpoint accepts Supabase service role key for local testing.
    This should be removed or disabled in production.
    """
    try:
        # ⚠️ TESTING ONLY: Check for service role key authentication FIRST
        auth_header = request.headers.get("Authorization", "")
        is_service_key_auth_result = is_service_key_auth(auth_header)
        
        if is_service_key_auth_result:
            print("🔑 Using service role key authentication for testing")
            pass
        else:
        ...
        if response.data:
            ...
            print(f"✅ Created event: {created_event['id']} - {event_name}")
...
async def get_project_events(
    ...
    try:
        ...
        print(f"✅ Retrieved {len(events)} events for project {project_id}")
        ...
        print(f"❌ Error getting project events: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def create_action_item(
    ...
    try:
        ...
        if response.data:
            ...
            print(f"✅ Created action item: {created_action['id']} - {action_name}")
...
async def get_action_item(
    ...
    try:
        ...
        print(f"❌ Error getting action item: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def update_action_item(
    ...
    try:
        ...
        if response.data:
            ...
            print(f"✅ Updated action item: {action_item_id}")
...
async def delete_action_item(
    ...
    try:
        ...
        if response.data:
            print(f"✅ Deleted action item: {action_item_id}")
...
async def get_project_action_items(
    ...
    try:
        ...
        print(f"✅ Retrieved {len(action_items)} action items for project {project_id}")
        ...
        print(f"❌ Error getting project action items: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
